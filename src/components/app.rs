...
pub enum Msg {
    ...
    OsdIndicatorSurface((SurfaceId, osd_indicator::Msg)),
        ...
        match message {
            ...
            Msg::CreateDisplayIdentifiers(displays) => {
                ...
                for (display_name, display_number) in displays {
                    ...
                    self.surfaces.insert(id, Surface::OsdIndicator(state));
                    tasks.push(cmd.map(move |msg| cosmic::action::app(Msg::OsdIndicatorSurface((id, msg)))));
                }
        ...
        if let Some(surface) = self.surfaces.get(&id) {
            return match surface {
                ...
                Surface::OsdIndicator(state) => {
                    state.view().map(move |msg| Msg::OsdIndicatorSurface((id, msg)))
                }
            };
